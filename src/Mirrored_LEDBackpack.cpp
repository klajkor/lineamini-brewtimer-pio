...
Adafruit_Y_Mirrored_8x16minimatrix::Adafruit_Y_Mirrored_8x16minimatrix(void) : Adafruit_GFX(8, 16) {
}

void Adafruit_Y_Mirrored_8x16minimatrix::drawPixel(int16_t x, int16_t y, uint16_t color) {
 ...
 // check rotation, move pixel around if necessary
  ...
  case 0:
    ...
    break;
  case 1:
    //y = 8 - y - 1; //<= not needed because of mirroring Y
    if (x >= 8) {
      x -= 8;
      y += 8; 
    }
  ...
  case 3:
    x = 16 - x - 1;
    y = 8 - y - 1;  //mirroring Y coordinate
    if (x >= 8) {
      x -= 8;
      y += 8; 
    }
